...
@app.route("/")
def home():
    recentideas = list(Idea.query.order_by(
        Idea.created_at.desc()).limit(3).all())
    strands = list(Strand.query.order_by(Strand.id).all())
    return render_template(
        "index.html", recentideas=recentideas, strands=strands)
...
def add_idea():
    ...
    subjects = ['Cymraeg', 'Saesneg', 'Ffrangeg', 'Sbaeneg', 'Hanes',
                ...
                'Tecstiliau', 'Graffeg', 'Addysg Gorfforol', 'Bwyd a Maeth',
                'ABCh', 'Bagloriaeth Cymru']
...
def about():
    ...
    return render_template(
        "about.html", strands=strands, categories=categories)
...
def full_idea(idea_id):
    ...
    if "user" not in session:
        ...
        recentideas = list(Idea.query.order_by(
            Idea.created_at.desc()).limit(3).all())
    return render_template(
        "full_idea.html", idea=idea, strands=strands, recentideas=recentideas,
...
def profile(username):
    ...
    if session["user"]:
        return render_template(
            "profile.html", ideas=ideas, strands=strands, username=username)
...
def search():
    if request.method == "POST":
        ...
        ideas = Idea.query.filter(
            Idea.idea_description.like('%' + searched + '%')).order_by(
                Idea.idea_name).all()
        subjects = ['Cymraeg', 'Saesneg', 'Ffrangeg', 'Sbaeneg', 'Hanes',
                    'Daearyddiaeth', 'Addysg Grefyddol', 'Busnes', 'Celf',
                    'Cerddoriaeth', 'Drama', 'Mathemateg', 'Bioleg', 'Cemeg',
...
def filter_ideas_by_query_param():
    ...
    if args.get("subject"):
        ...
        ideas = list(Idea.query.order_by(Idea.idea_name).all())
    return render_template(
        "ideas.html", subjects=subjects, strands=strands, ideas=ideas,
        categories=categories, camau_cynnydd=camau_cynnydd)
